...
- **Layout "boxed" (encaixotado) para header, footer e faixa de Concessionárias — inspirado no site oficial da Ford:** hoje esses elementos esticam 100% da largura da tela ("full-bleed"). O site oficial da Ford usa um layout diferente: header e footer ficam **contidos numa largura máxima, centralizados**, deixando uma margem visível nas laterais em telas grandes (onde aparece o fundo da página, não a cor do header/footer). Aplicar isso em `.cabecalho`, `.rodape` e na nova faixa de Concessionárias: `max-width: 1700px` + `margin: 0 auto`. **Importante — esse valor precisa ser EXATAMENTE igual ao `max-width` já usado em `.destaque-conteudo` (a imagem da Home)**, para as larguras baterem entre si. Os três elementos (faixa cinza, header branco, footer) devem compartilhar a mesma largura máxima entre si e com o conteúdo principal da página, para tudo ficar visualmente alinhado. Em telas pequenas (mobile, dentro da media query já existente), não é necessário manter esse efeito — pode continuar ocupando a largura toda, já que não há espaço sobrando pra mostrar a margem de qualquer forma.
- **Linha vertical fina conectando o header ao footer (top a bottom):** criar um elemento `<div>` novo, envolvendo TUDO que já existe dentro do `<body>` (faixa de Concessionárias, header, main, footer) — algo como `<div class="pagina-wrapper">`. Esse wrapper recebe `max-width: 1700px`, `margin: 0 auto`, e uma borda fina em ambos os lados: `border-left: 1px solid #e0e0e0` e `border-right: 1px solid #e0e0e0` (mesma cor cinza clara já usada no `border-bottom` do header). O resultado visual: uma linha vertical fina correndo dos dois lados da página inteira, desde o topo (faixa cinza) até a base (footer), "emoldurando" o conteúdo boxed. **Atenção técnica:** as propriedades de flexbox que hoje garantem o rodapé fixo na base (`display: flex; flex-direction: column; min-height: 100vh;`, hoje no `body`) precisam ser movidas para esse novo `.pagina-wrapper`, para o sticky footer continuar funcionando exatamente como antes.
- **Campo de busca do footer mais à esquerda:** hoje o campo de busca (`.rodape-busca`) está centralizado (`justify-content: center`). Mudar para `justify-content: flex-start`, deixando o campo próximo do canto esquerdo dessa faixa (o `padding` lateral que já existe evita que ele fique colado exatamente na borda).
...
- **Responsividade em 3 faixas — celular, tablet e notebook/desktop:** reestruturar a media query única que existe hoje (`@media (max-width: 600px)`) em **3 faixas separadas**, usando `min-width` + `max-width` combinados:
  - **Telas pequenas (celular):** `@media (min-width: 249px) and (max-width: 480px)` — mover pra cá as regras que já existem hoje na media query única (grid do header empilhado, `.destaque-conteudo` a 92%, `.rodape-wordmark` menor, logo do footer saindo do `position: absolute`, cards e contato empilhando).
  - **Telas médias (tablet):** `@media (min-width: 481px) and (max-width: 1024px)` — faixa **nova**, que hoje não tem nenhum ajuste dedicado. Adicionar regras intermediárias (nem o layout cheio de desktop, nem o empilhado de celular): reduzir gaps (`.cards-carros`, `.contato-colunas`), ajustar `.destaque-conteudo` pra uma largura intermediária (ex: 90%), e testar se `.contato-colunas` (imagem + formulário lado a lado) continua legível nessa largura ou se também precisa empilhar. Usar julgamento visual ao testar — os valores exatos podem ser ajustados livremente, o importante é o resultado ficar sem elementos cortados, sobrepostos ou espremidos.
  - **Telas grandes (notebook/desktop):** `@media (min-width: 1024px)` — **não precisa de regras novas**, já que os estilos padrão do CSS (sem media query) já são pensados pra essa faixa. Não criar um bloco vazio só por formalidade — se não houver nada específico a mudar, deixar um comentário explicando que essa faixa usa os estilos padrão, sem necessidade de bloco extra.
  - **Teste obrigatório:** validar as 3 faixas usando as Ferramentas de Desenvolvedor (F12), redimensionando a tela em pelo menos um valor representativo de cada faixa (ex: 375px, 768px, 1440px), navegando pelas 3 páginas do site. Se não houver uma ferramenta de automação de navegador disponível na sessão, revisar a lógica do CSS cuidadosamente e reportar ao aluno quais faixas foram ajustadas, pedindo que ele confirme visualmente no F12 antes de considerar concluído.
- **Preferir tags semânticas no lugar de `<div>` genérica sempre que o elemento representar conteúdo com significado próprio** (não apenas agrupamento visual/estilização pura). Fazer uma revisão geral no HTML das 3 páginas procurando `<div>` que poderiam virar `<section>`, `<article>`, `<aside>`, `<figure>` etc. Exemplos concretos de troca válida no projeto atual: o footer inteiro (`.rodape-container` envolvendo `.rodape-busca` + `.rodape`) pode virar uma única tag `<footer>` no lugar da `<div class="rodape-container">` externa, já que semanticamente tudo ali É o rodapé do site. `<div>` continua sendo a escolha certa para wrappers puramente estruturais/de estilo que não representam conteúdo (ex: `.pagina-wrapper`, `.contato-imagem-coluna`, `.contato-form-coluna`) — o objetivo não é eliminar todo `<div>`, é usar a tag mais apropriada quando ela existir e fizer sentido semântico.
